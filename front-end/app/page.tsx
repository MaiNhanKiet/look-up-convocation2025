...
export default function Home() {
  ...
  const validateMSSV = (value: string) => {
    ...
    const pattern = /^[SHCQD][ESA]([4-6]\d{3}|\d{6})$/
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (
        ...
        errorResponse?.message === 'Không tìm thấy sinh viên với Student ID'
      ) {
  ...
  const onSubmitMissingInfo = async (data: MissingInformationFormData) => {
    ...
    try {
      ...
      const errorResponse = error?.response?.data
      const errorMessage =
        errorResponse?.message || 'Đã xảy ra lỗi, vui lòng thử lại sau!'
          ...
          <div className="p-4 space-y-4">
            <DialogHeader className="p-0 pb-0">
              ...
              <div className="mt-2 text-sm italic text-gray-600 dark:text-gray-300">
                Sau khi nhận thông tin, <strong>Ban Tổ Chức</strong> sẽ tiến
                hành kiểm tra lại và liên hệ với bạn trong thời gian sớm nhất.
              </div>
            </DialogHeader>

            <Form {...missingInfoForm}>
              <form
                onSubmit={missingInfoForm.handleSubmit(onSubmitMissingInfo)}
                className="space-y-4"
              >
